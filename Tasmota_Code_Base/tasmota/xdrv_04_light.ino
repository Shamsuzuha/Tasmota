...
void LightInit(void)
{
  ...
  if (TasmotaGlobal.light_type < LT_PWM6) {           // PWM
    ...
    if (TasmotaGlobal.module_type == MJ_SD01_DIMMER) {   // setup GPIO for dimming status on faceplate
      pinMode(3, OUTPUT);
      pinMode(5, OUTPUT);
      pinMode(12, OUTPUT);
      pinMode(14, OUTPUT);
...
//  Light.power = XdrvMailbox.index;
  ...
  if (TasmotaGlobal.module_type == MJ_SD01_DIMMER) {  // adjust GPIO16 to turn on/off MCU power
    if (Light.power) {
      digitalWrite(16,LOW);
    } else {
...
//    cur_col_10[0], cur_col_10[1], cur_col_10[2], scale_col[0], scale_col[1], scale_col[2], max);
 ...
 if (TasmotaGlobal.module_type == MJ_SD01_DIMMER) {   // adjust display LEDs
    switch (cur_col[0]) {
      case 0 ... 50:
